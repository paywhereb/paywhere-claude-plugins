...
No Google Drive. Files (briefs, sweep records, the bank package — all markdown) are written
...
working folder for the demo (the skills write `briefs/`, `sweeps/` and
`bank/` under it).
...
2. Open the demo Cowork project (instructions = `cowork-project-prompt.md`;
   plugin 1.0.13 side-loaded; four connectors signed in as demo-nick).
...
| Skill "executed" or says "paid ✓" | Stale skill text | You are on an old plugin version; rebuild + side-load 1.0.13 |
...
| Cowork picked the wrong skill | Phrase too far from the description triggers | Use the exact SCENARIOS.md text; or name the skill |
| A beat runs long (> 30 s, > 6 calls) | Skill drifted from its call plan | Compare the tool calls with the skill's Quick start; report it — the eval budgets every case |
...
The Meridian Staffing world is kept in the repos for reference and is **not
maintained**; its books are no longer reseeded, and its `pay-and-bill` /
`pay-commissions` skills left the plugin package in 1.0.8 (git history before
that). The old script is at the bottom of
