...
In Cowork use the "side-load a plugin file" picker and choose the `.plugin`
file. Then connect the four connectors in Cowork's connector settings and
sign in as above. Pick a working folder for the demo (the skills write
`briefs/`, `sweeps/`, `dashboard/`, `models/`, `bank/`, `close/` under it).
...
2. `/demo-setup` (≈ 5 min). Record credentials. Check the four readback ✓.
3. Sign the bare-connector window in with the new bank user.
4. Pre-run Act 3 (`Run my morning cash brief`, `Run the Friday tax sweep`)
   so files and proposals exist; do **not** approve them yet.
5. Open paywhere-admin on Intents.
6. Walk SCENARIOS.md. Approve proposals live on the bank's page.
7. After: re-run `/demo-setup` if you injected; leave proposals unapproved
   or cancel them.
...
| Skill "executed" or says "paid ✓" | Stale skill text | You are on an old plugin version; rebuild + side-load 1.0.0 |
