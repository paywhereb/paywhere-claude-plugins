...
**Paywhere transactions already in QB** — if the user has a Paywhere → QB
bank feed enabled, Paywhere lines will already appear in the QB register
with the Paywhere transaction id in the memo. Don't double-count them during
reconciliation (Step 3); match by id rather than by amount+date when the
memo is populated.
