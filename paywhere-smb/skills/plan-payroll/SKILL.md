---
name: plan-payroll
description: Forecasts cash from QuickBooks AR/AP and Paywhere balances, ranks overdue invoices, and stages mail-draft reminders so the owner can confidently run payroll. Accepts optional horizon and payroll-date arguments.
...
1. Pull AR, AP, and historical cash timing from QuickBooks, plus current balances and recent inflow/outflow from Paywhere (`list_accounts` → `get_account_balance` and `get_account_transactions`). Fall back to CSV upload if no connector is live.
...
1. Pull overdue invoices from QuickBooks; cross-reference Paywhere credits from the last 14 days to suppress customers who already paid (see `invoice-chase` for the matching logic).
...
4. Queue every reminder as a Mail draft. The owner sends from their mail client — this skill never sends directly.
...
- If a connector is unreachable (QuickBooks, Paywhere, Mail), stop, report which connector failed, and ask whether to retry, fall back to CSV, or abort.
