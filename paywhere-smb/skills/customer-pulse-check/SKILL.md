...
description: Synthesizes themes from HubSpot tickets, Intercom conversations, Gmail threads, and review exports into a top-3 fixable issues list with drafted response templates. Accepts optional since-date argument.
...
1. Pull HubSpot support tickets and conversation notes for the period.
2. Pull Intercom conversations (customer messages only) and Gmail threads matching the seed keyword list from `customer-pulse`.
...
Total signals: {n} ({HubSpot tickets: n} | {Intercom: n} | {Gmail: n} | {Reviews: n})
...
Run with whatever sources are connected — this command degrades gracefully. If HubSpot is missing, skip ticket data and note it. If Intercom is missing, skip conversation data. If Gmail is missing, skip keyword scan. If no sources are connected at all, stop and tell the owner: "No feedback sources connected. Connect at least one of HubSpot, Intercom, Gmail — or upload a review export CSV."

## Approval gates

- **Never send response emails automatically.** Present drafts for owner review only.
- **Never close HubSpot tickets or modify any customer record without explicit owner confirmation.**
