...
When the owner doesn't have QuickBooks connected, they can upload exported CSV files. This document specifies what columns to expect and how to handle variations.
...
- QuickBooks: Reports → Sales by Product/Service Detail → Export
- Shopify: Orders → Export
- Paywhere: Activity → Export per account (best when paired with QB invoice data; bank lines alone lack product-level granularity)
...
- Paywhere: Activity → Export (debit lines, grouped by counterparty)
