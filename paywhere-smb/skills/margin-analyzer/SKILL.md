...
description: >
  Analyzes unit economics by product or service using QuickBooks revenue
  and cost data, benchmarks against inflation and cost changes, and shows
  pricing-scenario data (e.g. "a 5% increase historically correlates with
  ~3% volume drop"). Surfaces analysis only — does not recommend a price.
  Use when the user asks about raising prices, pricing, margin analysis,
  what to charge, whether costs are eating into profit, or how a price
  change might affect their business. Trigger even if the user doesn't say
  "margin" explicitly — phrases like "am I making enough?", "should I
  charge more?", or "my costs are going up" all call for this skill.
---
...
3. **Pulls revenue data** from QuickBooks invoice line items
...
**No connector:** Offer CSV upload as a fallback. The skill can work from exported transaction and expense data. The expected CSV schema is in `reference/csv-schema.md`.
...
### Step 4: Pull revenue data (QuickBooks)

Fetch invoice line items from QuickBooks for the cost-data window (last 12 months):

- **Extract:** Transaction amount, item/service name, date, quantity if available
- Group by product/service code so revenue can be paired 1:1 with COGS in Step 5.

If QuickBooks is unavailable, fall back to the CSV path described in Step 3 — ask for sales-by-product (date, item, quantity, unit price) for the last 12 months. Note the limitation in the output.
...
**Primary:** QuickBooks
**Also supported:** Desktop (CSV/export)
