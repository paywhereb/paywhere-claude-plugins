...
→ Pull all contractor/vendor payments from QuickBooks (and cross-check Paywhere ACH/wire history)
...
3. **1099-NEC candidates table** — Columns: payee name, total paid, data
   sources (QB / Paywhere ACH / Paywhere wire / CSV), W-9 status (on file /
   missing / unknown), and notes.
...
6. **Paywhere reconciliation note** — Surface any contractor payment found in
   Paywhere (ACH/wire) that did not appear in QuickBooks vendor records, so
   the accountant can confirm whether a missed 1099 obligation exists.
...
7. **Next steps checklist** — Action items for the accountant: collect missing
   ...
   exemptions, reconcile any Paywhere-only contractor payments, file by January 31.
...
- [reference/connector-queries.md](reference/connector-queries.md) — how to pull data from QuickBooks and Paywhere
