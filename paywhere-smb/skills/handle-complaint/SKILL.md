...
2. If the customer's payment history is on the operating bank account, cross-check Paywhere `get_account_transactions` for any credit matching their invoice amount in the last 30 days — confirms whether they actually paid (and when).
...
If Gmail and HubSpot are both unreachable, ask the owner to paste the complaint text — the skill works with manual input. If Paywhere is missing, skip the bank-side payment cross-check and note "Paywhere not connected — working from complaint text and HubSpot history only."
