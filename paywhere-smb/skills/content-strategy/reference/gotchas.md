...
## Gotcha: QuickBooks not connected and the owner has no CSV ready

If QuickBooks is unreachable and the owner can't immediately upload a CSV, the skill can't pull product-level sales data.

**Why it matters:** A content brief built from gut alone is what the owner already has. The skill exists to layer data over instinct — without data, it adds nothing.

### ✗ Bad
Proceed with generic "post about your best seller" recommendations.

### ✓ Good
Pause cleanly: "I need product-level sales data to recommend anything specific. Either connect QuickBooks (one-time setup, takes 2 minutes) or export your last 90 days of sales as CSV from whatever system you use. Want to do that, or should we come back to this?"
