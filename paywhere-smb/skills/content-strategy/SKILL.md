...
description: >
  Analyzes sales data from QuickBooks (invoice line items, product/service
  revenue by date) to find top performers and slow movers, layers in
  seasonality, and produces a prioritized 30-day content brief: what to
  push, what offers to run, what to hold. Strategic output only — no
  calendars or assets. Use when the user asks what to post, wants a content
...
1. **Pulls sales data** from QuickBooks (invoice line items, product/service revenue by date)
...
**Fallback:** If <3 months of data, use industry seasonality benchmarks for the SMB's category (e.g., retail, services, e-commerce). If QuickBooks isn't connected, ask the owner to upload a CSV of sales-by-product for the last 90 days.
