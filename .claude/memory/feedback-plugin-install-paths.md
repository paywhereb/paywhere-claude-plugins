...
**Packaging:** `./scripts/package.sh` (in repo root) takes the version from `paywhere-smb/.claude-plugin/plugin.json` and writes `dist/paywhere-smb-<version>.plugin` and `dist/paywhere-smb-<version>.zip`, plus `dist/paywhere-smb-<version>-poc.plugin` whenever the plugin's `.mcp.json` names the hosted demo Paywhere MCP (same contents, Paywhere server renamed "Paywhere POC" and its URL swapped for `PAYWHERE_POC_URL`, default `https://paywhere-mock-mcp.poc.dev.paywhere.com/mcp`; added 2026-09-03 so a PoC build is always one side-load away. The rename matters: Desktop has no "remove connector", and a second server named "Paywhere" would not install next to the first). The `dist/` directory is gitignored — artifacts are built locally or attached to GitHub releases, not tracked in git.
